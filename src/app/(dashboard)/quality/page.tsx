...
const QualityManagement = () => {
  ...
  const handleStatusFilter = (status: string | null) => {
    ...
    router.replace(`/quality?${params.toString()}`);
  };
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    } catch { toast.error("Erro ao reordenar"); fetchProjects(); }
  };
  ...
  const handleStatusChange = async (projectId: string, newStatus: string) => {
    try {
      ...
      if (error) throw error;
      toast.success("Status atualizado!"); fetchProjects();
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <div className={`grid gap-6 ${statusFilter ? "grid-cols-1" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-6"}`}>
              {statusCards.filter(s => !statusFilter || statusFilter === s.key).map(s => (
                <ProjectColumn key={s.key} title={s.label} status={s.key} projects={s.projects}
