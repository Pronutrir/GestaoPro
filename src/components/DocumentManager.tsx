...
export const DocumentManager = ({ projectId, phases, activities }: DocumentManagerProps) => {
  ...
  const handleSubmit = async () => {
    ...
    setForm({ file_name: "", file_url: "", file_type: "", description: "", uploaded_by: "", phase_id: "", activity_id: "" });
    setShowForm(false);
    fetchDocuments();
    ...
    <Card className="p-6 space-y-4">
      ...
      {showForm && (
        <div className="space-y-3 p-4 bg-accent/30 rounded-lg border border-border">
          ...
          />
          <Input
            placeholder="URL do documento * (link do Google Drive, OneDrive, etc.)"
            value={form.file_url}
            onChange={(e) => setForm({ ...form, file_url: e.target.value })}
          />
          <div className="grid grid-cols-2 gap-3">
            <Input
              placeholder="Tipo (PDF, DOCX, etc.)"
