-- ============================================================================
-- A FATIA CIRURGICA — 68 folhas no quadro voltam a ser 'atividade'
--
-- INCIDENTE 27/08/2026. Conserto PARA A FRENTE: nao reverte build, nao aplica
-- o congelamento, nao mexe na deducao por nivel nem nos 771.
--
-- ----------------------------------------------------------------------------
-- O QUE SAO OS 68
--
-- Itens VIVOS, no quadro (coluna nao-backlog), SEM filhas vivas, e exibidos
-- como agrupador. Ou seja: folhas de trabalho que o quadro nao desenha como
-- cartao. O trabalho existe, esta promovido, e ninguem o ve.
--
-- Medidos em TODOS os projetos (17 afetados), nao so no de teste:
--   docs/medicoes/quadro-presos-e-faixas-27-08-2026.md
--   docs/medicoes/quadro-presos-e-faixas-27-08-2026.csv
--
-- Todos gravados como 'fase' hoje. A sombra separa dois casos:
--   11 tem sombra='atividade' -> o pacote_e_posicao os converteu em 24/08;
--      estes VOLTAM ao que eram.
--   57 tem sombra='fase' ...... sempre foram fase; estes MUDAM de verdade, e e
--      justificado: sao folhas, sao trabalho, e nunca deveriam ter sido fase.
--
-- ----------------------------------------------------------------------------
-- O QUE ESTA MIGRATION NAO E
--
-- NAO e o backfill do congelamento. E uma fatia dele, limitada a quem esta no
-- quadro e sem filhas — 68 linhas com id explicito, nao 2.604 por predicado.
-- O congelamento retomavel continua na fila e nao e afetado.
--
-- NAO TOCA NA SOMBRA. A coluna item_type_antes_congelar fica intacta: ela e o
-- unico registro do que havia antes do pacote_e_posicao, e o rollback desta
-- migration depende de ela existir com o valor de hoje.
--
-- NAO MEXE NOS 115 AGRUPADORES no quadro. Eles tem filhas no quadro e desenham
-- faixa sobre elas — e assim que o agrupamento funciona. Devolve-los ao backlog
-- deixaria 456 cartoes soltos.
--
-- ----------------------------------------------------------------------------
-- IDS EXPLICITOS, e nao um WHERE
--
-- Um predicado ("no quadro E sem filhas E agrupador") pegaria tambem o que
-- mudar entre a medicao e a execucao — alguem promovendo um item, alguem
-- apagando a ultima filha de um agrupador. A lista foi medida num instante
-- conhecido, e e ela que vale.
--
-- ROLLBACK: 20260827140001 — mesmos ids, valores originais.
-- ============================================================================
...
DO $antes$
DECLARE v_faltam int; v_mudou int; v_total int;
BEGIN
  SELECT count(*) INTO v_total FROM _alvo;
  IF v_total <> 68 THEN
    RAISE EXCEPTION 'a lista tem % ids, esperava 68', v_total;
  END IF;

  SELECT count(*) INTO v_faltam
    FROM _alvo t LEFT JOIN public.activities a ON a.id = t.id WHERE a.id IS NULL;
  IF v_faltam > 0 THEN
    RAISE EXCEPTION '% dos 68 ids nao existem mais na tabela', v_faltam;
  END IF;

  -- Se algum ja nao esta como foi medido, alguem mexeu no meio. Melhor parar.
  SELECT count(*) INTO v_mudou
    FROM _alvo t JOIN public.activities a ON a.id = t.id
   WHERE a.item_type IS DISTINCT FROM t.item_type_antes;
  IF v_mudou > 0 THEN
    RAISE EXCEPTION '% item(ns) mudaram desde a medicao — refaca a medicao', v_mudou;
  END IF;
END $antes$;

-- ---------------------------------------------------------------------------
-- 2) A ESCRITA — so item_type, so os 68
--
-- is_milestone nao e tocado (nenhum dos 68 e marco, e virar marco seria outra
...
UPDATE public.activities a
  ...
  FROM _alvo t
 WHERE t.id = a.id AND a.item_type IS DISTINCT FROM 'atividade';
...
BEGIN
  ...
  IF v_fora > 0 THEN
    RAISE EXCEPTION '% dos 68 nao viraram atividade', v_fora;
  END IF;
  ...
  -- Os 115 agrupadores legitimos nao estao na lista, mas conferir e barato e o
  -- erro seria caro.
  ...
  RAISE NOTICE 'agrupadores com filhas vivas que seguem como fase: %', v_faixas;

  RAISE NOTICE 'os 68 viraram atividade; sombra intacta';
END $depois$;

-- CONFIRA ACIMA E ENTAO:  COMMIT;  ou  ROLLBACK;
--
-- Esperado: ANTES  68 como 'fase' (11 com sombra 'atividade', 57 com 'fase')
--           DEPOIS 68 como 'atividade'
--
-- Deixado ABERTO de proposito: quem executa olha os numeros antes de gravar.
